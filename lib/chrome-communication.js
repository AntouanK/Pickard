...
API.openSocket = function(thisPickard, wsURL){
  ...
  ws.on('message', function(data/*, flags*/) {
    ...
    if(message !== undefined){
      ...
      if(!contextCreated && message.method === 'Runtime.executionContextCreated'){
      ...
      } else if(!loadEventFired && message.method === 'Page.loadEventFired'){
      ...
      if(message.method === 'Page.Page.frameStartedLoading'){
        thisPickard.showDebug && log.warning(message);
      }
...
API.launch = function(config){
  ...
  //  OSX:
  if(process.platform === 'darwin'){
    args.push('--user-data-dir="~/Library/Application\ Support/Google/Chrome/Default/"');
  }
