...
/*
  ...
  - document.readyState
  check of "load" event ( document.readyState : "complete", "loading")
  ...
  ws.on('message', function(data, flags) {
    ...
    if(dataJSON !== undefined){
      ...
      if(!contextCreated && dataJSON.method === 'Runtime.executionContextCreated'){
        ...
        thisPickard.pageLoadTime = process.hrtime(pageNavigateTime);
        log.warning('loadEventFired');
        pageLoadDeferred.resolve();
        loadEventFired = true;
        log.data('page load time', prettyTime(thisPickard.pageLoadTime));

      } else if(!loadEventFired && dataJSON.method === 'Page.frameNavigated'){

        pageNavigateTime = process.hrtime();
      }
